...
void fft(cf64* __restrict__ wave, cf64* __restrict__ roots, u64 n){
	...
	for(u64 logp = 1; logp <= logn; ++logp){
		...
		u64 halfp = p >> 1;
		for(u64 i = 0; i < n; i += p){
			for(u64 j = 0; j < halfp; j += vector_step/2){ // step diviso 2 perché sono coppie
				svbool_t predicate = svwhilelt_b64(2*j, p); // doppio perché se mi mancano k complessi sono 2k double
				// indici "pari" e "dispari" corrispondono alla versione ricorsiva di fft
				u64 even = i+j, odd = i+j+halfp;

				// contazzi per trovare gli indici delle radici giuste
				svuint64_t root_indices = svindex_u64(2*j, 1);
				root_indices = svlsr_x(predicate, root_indices, 1); // j j j+1 j+1 j+2 j+2 ...
				root_indices = svlsl_x(predicate, root_indices, logn-logp+1); // *= step angoli, *= 2
				root_indices = svorr_x(predicate, root_indices, sv_0101); // rendi dispari i dispari

				svfloat64_t w = svld1_gather_index(predicate, roots_data, root_indices);
				svfloat64_t wave_even = svld1(predicate, (f64*)(&wave[even]));
				svfloat64_t wave_odd = svld1(predicate, (f64*)(&wave[odd]));
				// z = w * odd
				svfloat64_t z = svdup_f64(0);
				z = svcmla_x(predicate, z, w, wave_odd, 0);
				z = svcmla_x(predicate, z, w, wave_odd, 90);
				// odd := even - z = even - w * odd; even := even + z = even + w * odd
				svst1(predicate, (f64*)&wave[odd], svsub_x(predicate, wave_even, z));
				svst1(predicate, (f64*)&wave[even], svadd_x(predicate, wave_even, z));
			}
		}
	}
}
...
int main(int argc, char const * argv[]){
	...
	init_roots(roots, n);using std::chrono::high_resolution_clock;
